...
class N4QuestionSeeder extends Seeder
{
    /**
     * Import the real JLPT N4 questions from plain-text banks.
     */
    public function run(): void
    {
        $category = ExamCategory::where('name', 'JLPT')->first();
        $level = $category
            ? ExamLevel::where('category_id', $category->id)->where('code', 'N4')->first()
            : null;

        if (! $level) {
            $this->command?->warn('N4QuestionSeeder: JLPT not found — run ExamSeeder first. Skipped.');
            return;
        }

        $banks = [
            'kanji'   => base_path('knowledge/N4kanji.txt'),
            'vocab'   => base_path('knowledge/N4vocabulary.txt'),
            'grammar' => base_path('knowledge/N4grammar.txt'),
        ];

        foreach ($banks as $section => $path) {
            if (! is_file($path)) {
                $this->command?->warn("N4QuestionSeeder: {$path} missing — skipped {$section}.");
                continue;
            }
            ...
            $this->command?->info("N4QuestionSeeder: imported {$importedCount} {$section} questions.");
        }
    }
