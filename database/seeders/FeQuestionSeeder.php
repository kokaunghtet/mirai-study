...
class FeQuestionSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        $level = $category
            ? ExamLevel::where('category_id', $category->id)->where('code', 'FE')->first()
            : null;

        if (! $level) {
            $this->command?->warn('FeQuestionSeeder: ITPEC/FE not found — run ExamSeeder first. Skipped.');
        ...
        foreach ($banks as $section => $path) {
            if (! is_file($path)) {
                $this->command?->warn("FeQuestionSeeder: {$path} missing — skipped {$section}.");
            ...
            $this->command?->info('FeQuestionSeeder: imported '.count($questions)." {$section} questions.");
        }
    }
    ...
    {
        $raw = preg_replace('/\R/', "\n", $raw);
        // One chunk per question: split right before each "Q<n>." line.
        $chunks = preg_split('/(?=^\s*Q\d+\s*\.)/m', $raw, -1, PREG_SPLIT_NO_EMPTY);

        $out = [];

        foreach ($chunks as $chunk) {
            if (! preg_match('/Ans\s*:\s*\(?\s*([A-Da-d])\s*\)?/', $chunk, $m)) {
