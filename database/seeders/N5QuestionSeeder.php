...
class N5QuestionSeeder extends Seeder
{
    /**
     * Import the real JLPT N5 questions from plain-text banks.
     */
    public function run(): void
    {
        $category = ExamCategory::where('name', 'JLPT')->first();
        $level = $category
            ? ExamLevel::where('category_id', $category->id)->where('code', 'N5')->first()
            : null;

        if (! $level) {
            $this->command?->warn('N5QuestionSeeder: JLPT not found — run ExamSeeder first. Skipped.');
            return;
        }

        $banks = [
            'kanji'   => base_path('knowledge/jlpt/N5kanji.txt'),
            'vocab'   => base_path('knowledge/jlpt/N5vocabulary.txt'),
            'grammar' => base_path('knowledge/jlpt/N5grammar.txt'),
        ];

        foreach ($banks as $section => $path) {
            if (! is_file($path)) {
                $this->command?->warn("N5QuestionSeeder: {$path} missing — skipped {$section}.");
                continue;
            }
            ...
            $this->command?->info("N5QuestionSeeder: imported {$importedCount} {$section} questions.");
        }
    }
