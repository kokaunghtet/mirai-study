...
class N2QuestionSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        $level = $category
            ? ExamLevel::where('category_id', $category->id)->where('code', 'N2')->first()
            : null;
        ...
        $banks = [
            'kanji' => base_path('wiki/jlpt/N2kanji.txt'),
            'vocab' => base_path('wiki/jlpt/N2vocabulary.txt'),
            'grammar' => base_path('wiki/jlpt/N2grammar.txt'),
        ];
        ...
        foreach ($banks as $section => $path) {
            ...
            $this->command?->info('N2QuestionSeeder: imported '.count($questions)." {$section} questions.");
        }
    }
    ...
    private function parse(string $raw): array
    {
        ...
        if (preg_match_all($pattern, $raw, $matches, PREG_SET_ORDER)) {
            foreach ($matches as $match) {
                ...
                // Highly flexible choice delimiters supporting both "a:" and "a)" formats
