...
class N1QuestionSeeder extends Seeder
{
    /**
    ...
    public function run(): void
    {
        ...
        $level = $category
            ? ExamLevel::where('category_id', $category->id)->where('code', 'N1')->first()
            : null;

        if (! $level) {
            $this->command?->warn('N1QuestionSeeder: JLPT not found — run ExamSeeder first. Skipped.');
            return;
        }

        $banks = [
            'kanji'   => base_path('knowledge/jlpt/N1kanji.txt'),
            'vocab'   => base_path('knowledge/jlpt/N1vocabulary.txt'),
            'grammar' => base_path('knowledge/jlpt/N1grammar.txt'),
        ];

        foreach ($banks as $section => $path) {
            if (! is_file($path)) {
                $this->command?->warn("N1QuestionSeeder: {$path} missing — skipped {$section}.");
                continue;
            }
            ...
            $this->command?->info("N1QuestionSeeder: imported {$importedCount} {$section} questions.");
        }
    }
