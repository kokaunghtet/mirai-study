            ...
            <div x-show="mobileSheet"
                ...
                <div class="py-2">
                    ...
                    <div class="px-5 pt-2 pb-1">
                        <p class="pb-1 text-[10px] font-semibold uppercase tracking-wide text-muted">Accounts</p>
                        @foreach ($switchableAccounts as $account)
                            @if ($account->id === auth()->id())
                                <div class="flex items-center gap-3 overflow-hidden rounded-lg bg-accent/10 px-2 py-2">
                                    <div class="h-8 w-8 shrink-0 rounded-full bg-accent/15 flex items-center justify-center text-xs font-bold text-accent overflow-hidden">
                            ...
                            @else
                                <div class="flex items-center gap-1 overflow-hidden">
                                    <form method="POST" action="{{ route('accounts.switch', $account) }}" class="min-w-0 flex-1">
                                        @csrf
                                        <button type="submit" class="flex w-full items-center gap-3 rounded-lg px-2 py-2 text-left hover:bg-surface-muted transition-colors">
                                            <div class="h-8 w-8 shrink-0 rounded-full bg-accent/15 flex items-center justify-center text-xs font-bold text-accent overflow-hidden">
                                                @if ($account->profile_image)
                                                    <img src="{{ $account->profile_image }}" alt="{{ $account->display_name }}" class="h-full w-full object-cover">
                                                @else
                                    ...
                                    <form method="POST" action="{{ route('accounts.remove', $account) }}">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @if ($switchableAccounts->count() < \App\Services\LinkedAccountService::MAX_ACCOUNTS)
                            <a href="{{ route('accounts.add') }}"
                               @click="mobileSheet = false"
                               class="flex items-center gap-3 rounded-lg px-2 py-2 text-sm font-medium text-accent hover:bg-surface-muted transition-colors">
                ...
                <div class="relative" x-data="{ userMenu: false }">
                    ...
                    <div x-show="userMenu"
                        ...
                        <div class="px-2 pb-1 pt-1.5">
                            ...
                            @foreach ($switchableAccounts as $account)
                                @if ($account->id === auth()->id())
                                    <div class="flex items-center gap-2.5 overflow-hidden rounded-lg bg-accent/10 px-2 py-2">
