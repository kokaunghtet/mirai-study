        ...
        <div class="flex items-center gap-2.5">
            ...
            <div>
                <a href="{{ route('profile.show', $post->user->username) }}"
                   class="text-[13px] font-bold text-content hover:text-accent transition-colors truncate max-w-[10rem] lg:max-w-none inline-block">
                ...
                <div x-show="open"
                    ...
                    @else
                        @if (auth()->user()?->isAdmin() || auth()->user()?->isModerator())
                            <form method="POST" action="{{ route('posts.destroy', $post) }}"
                                  data-confirm="Remove this post?" data-loading>
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                        class="w-full px-3 py-2 text-left text-red-600 hover:bg-red-50 transition">
